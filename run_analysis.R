...
colnames(df_subset_Mean_std)

#3.Uses descriptive activity names to name the activities in the data set

df_subset_Mean_std$activity <- factor(df_subset_Mean_std$activity, levels = activities[,1], labels = activities[,2])
...
df_subset_Mean_std_renamed = renameDF(df_subset_Mean_std)

str(df_subset_Mean_std_renamed)

#5. From the data set in step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject.
tidy_dataset = df_subset_Mean_std_renamed%>%
